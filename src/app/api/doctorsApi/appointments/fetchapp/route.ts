...
export async function POST(req: NextRequest) {
  try {
    await connectDB();
    console.log(
      "Connected to MongoDB. Models registered:",
      mongoose.modelNames()
    );
    ...
    const rawAppointments = await Appointment.find()
      .where("doctor")
      .equals(new mongoose.Types.ObjectId(doctorId))
      .populate({
        path: "user",
        model: "user",
        select: "username email status mode",
      })
      .populate({
        path: "doctor",
        model: "doctors",
        select: "name specialty rate",
      })
      .lean();

    const appointments = rawAppointments.filter((appt) => appt.user !== null);
